...
namespace VikingSagaWpfApp.Code.BattleNs
{
    class AwesomeAiPlayer : Player
    {
        ...
        private IEnumerable<CardTargetFlags> GeneratePotentialTargets(string cardOrdering, int maxPerCardOrdering)
        {
            _cardTargetGenerator.Reset(cardOrdering.Length);
            CardTargetFlags boardMobFlags = 0;
            ...
            while (true)
            {
                ...
                while (i < cardOrdering.Length)
                {
                    ...
                    if (BattleCard.RowOFlags.HasFlag(currentTarget))
                    {
                    ...
                    if (BattleCard.RowEFlags.HasFlag(currentTarget))
                    {
                    ...
                    if (isMob && currentTarget != CardTargetFlags.Null)
                    {
        ...
        private void ExecutePotentialTargets(string cardOrdering, List<CardTargetFlags> potentialTargets)
        {
            int i = 0;
            while (i < potentialTargets.Count)
            {
                ...
                //   3) Execute combat
                _testBattle.DoCombat(_testBattle.FirstPlayer, _testBattle.SecondPlayer);
        ...
        private AiPlay CreateAiPlay(
            string cardOrdering,
            float score,
            IEnumerable<CardTargetFlags> targets,
